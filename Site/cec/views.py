...
def entry_edited(response,id):
    if response.method == "POST": 
        ...
        if form.is_valid():
            ...
            obj.save()
            print("modified")
            return redirect("/carbon_emissions_calculator/admin_entry/")
    else:
        form = ModifyEntry()
...
def user_changed(response,name):
    if response.method == "POST": 
        ...
        if form.is_valid():
            ...
            obj.save()
            print("modified")
            return redirect("/carbon_emissions_calculator/admin_user/")
    else:
        form = ChangeUserAttributes()
    return render(response, "cec/entry_edited.html")
...
def survey_changed(response,title):
    if response.method == "POST": 
        ...
        if form.is_valid():
            ...
            obj.save()
            return redirect("/carbon_emissions_calculator/admin_survey/")
    else:
        form = ChangeSurvey()
...
def entry_organize_redirect(response):
    if response.method == "POST": 
        ...
        if form.is_valid():
            ...
            return redirect("/carbon_emissions_calculator/admin_page/entry_organize/survey="+str(title)+"/transportation="+str(transportation)+"/review="+str(review)+"/category="+str(category)+"/order="+str(order)+"/")
    else:
        form = OrganizeEntries()
